...
function isAnswerCorrect(q: QuizQuestion, a: AnswerState[string] | undefined): boolean {
  if (!a) return false
  if (q.type === 'matching') {
...
const countCorrect = (questions: QuizQuestion[], answers: AnswerState): number =>
  questions.reduce((acc, q) => (isAnswerCorrect(q, answers[q.id]) ? acc + 1 : acc), 0)
      ...
      {pairs.map((p, i) => {
        ...
        return (
          <div key={p.left} className="flex flex-col sm:flex-row sm:items-center gap-2">
            ...
            {showMobileHint && <span className="text-center text-gray-600 text-xs sm:hidden">matches ↓</span>}
            <select
              disabled={disabled}
              value={chosen ?? ''}
              onChange={(e) => {
...
export function QuizEngine({ quizId, questions, title, onComplete }: QuizEngineProps) {
  ...
  const correctCount = useMemo(() => countCorrect(questions, answers), [answers, questions])

  useEffect(() => {
    if (submitted && onComplete) onComplete(correctCount, questions.length)
  }, [submitted, correctCount, questions.length, onComplete])

  const checkAnswer = useCallback((q: QuizQuestion): boolean => isAnswerCorrect(q, answers[q.id]), [answers])
...
export function ExamEngine({ examId, title, questions, minutes, onFinish }: ExamEngineProps) {
  ...
  const submitNow = useCallback(() => {
    ...
    // The ref must stay in sync with state so the timer loop never double-submits.
    setSubmitted(true)
  ...
  const lowTime = started && minutes !== null && !submitted && secondsLeft < LOW_TIME_WARNING_SECONDS
  ...
  const isAnsweredCorrect = useCallback(
    (q: QuizQuestion): boolean => isAnswerCorrect(q, answers[q.id]),
    [answers]
  )
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-navy-600/40">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {minutes === null ? 'Untimed' : formatClockTime(secondsLeft)}
          </span>

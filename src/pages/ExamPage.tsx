...
import { ExamEngine } from '../components/engines'
...
function buildPerformance() {
  ...
  const weak = rows
    .filter((r) => r.attempts > 0 && r.pct < WEAK_THRESHOLD)
...
function PerformanceDashboard({ performance }: { performance: ReturnType<typeof buildPerformance> }) {
  return (
    <section className="mb-10">
      ...
      <p className="text-sm text-gray-400 mb-5">
        Built from every quiz and mock exam you have taken on this device. Categories below {WEAK_THRESHOLD}% are flagged so you
        ...
        <>
          <div className="grid sm:grid-cols-3 gap-3 mb-6">
            <div className="glass-panel p-4">
              <p className="text-2xl font-mono font-bold text-white">
                {performance.totals.attempts}
              </p>
              <p className="text-xs text-gray-400 mt-1">attempts recorded</p>
            </div>
            <div className="glass-panel p-4">
              <p className="text-2xl font-mono font-bold text-emerald-400">
                {performance.totals.totalQuestions ? Math.round((performance.totals.totalCorrect / performance.totals.totalQuestions) * 100) : 0}%
              </p>
              <p className="text-xs text-gray-400 mt-1">overall accuracy ({performance.totals.totalCorrect}/{performance.totals.totalQuestions})</p>
            </div>
            <div className="glass-panel p-4">
              <p className="text-2xl font-mono font-bold text-cyan-400">{performance.totals.topicsDone}</p>
          ...
          <div className="space-y-2 mb-6">
            {performance.rows.map((r) => (
              <div key={r.categoryId} className="glass-panel p-3.5">
                <div className="flex flex-wrap items-center justify-between gap-2 mb-1.5">
                  <span className="text-sm font-semibold text-gray-200">{r.label}</span>
                  <span className={`text-xs font-mono ${r.pct < WEAK_THRESHOLD ? 'text-crimson-400' : r.pct < STRONG_THRESHOLD ? 'text-amber-400' : 'text-emerald-400'}`}>
                ...
                <div className="h-1.5 rounded-full bg-navy-800 overflow-hidden">
                  <div
                    className={`h-full rounded-full ${
                      r.pct < WEAK_THRESHOLD ? 'bg-crimson-500' : r.pct < STRONG_THRESHOLD ? 'bg-amber-500' : 'bg-emerald-500'
                    }`}
          ...
          {performance.weak.length > 0 && (
            <div className="glass-panel p-5 border-crimson-600/40">
              ...
              <div className="space-y-3">
                {performance.weak.map((w) => {
                  const topics = getTopicsByCategory(w.categoryId as Category).slice(0, 3)
                  return (
...
export default function ExamPage() {
  ...
  const performance = useMemo(() => buildPerformance(), [performanceVersion, session])
